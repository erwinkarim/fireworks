class MachinesController < ApplicationController
  ...
  def gen_features
    ...
    @features = @machine.features.where{ (id.lteq start_id) }.order('features.id desc').limit(1000)
    #@features = @machine.features.where{ (id.lteq start_id) }.order('features.created_at asc')
